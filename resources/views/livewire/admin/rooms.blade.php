<div class="space-y-4">
    <div class="flex items-center justify-between">
        <h1 class="text-xl font-bold">Salas</h1>
        <x-button label="Nova" icon="o-plus" link="{{ route('admin.rooms.create') }}" class="btn-primary btn-sm" />
    </div>
    ...
    <x-table :headers="$headers" :rows="$rows" with-pagination per-page="perPage" :per-page-values="[10,25,50]" :sort-by="$sortBy" show-empty-text>
        ...
        @scope('actions', $row)
        <x-button icon="o-pencil-square" class="btn-xs" link="{{ route('admin.rooms.create', ['id'=>$row->id]) }}" />
        @endscope
    </x-table>
</div>
